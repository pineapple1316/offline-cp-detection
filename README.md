...
We propose a signflip parallel analysis based threshold selection procedure. When there is no change point, $\boldsymbol{w}$ and $\tilde{\boldsymbol{w}}$ show similar patterns; when the change point exists, $\boldsymbol{w}$ is always bigger than $\tilde{\boldsymbol{w}}$. We can also see $\tilde{\boldsymbol{w}}$ remains stable no matter a change point exists or not, showing the signflip procedure can break the correlation-change signal.
